...
//thanks to all spanish and turkish pasters
...
{
	uint64_t bonearray = read<uint64_t>(mesh + 0x518);
	if (!bonearray) bonearray = read<uint64_t>(mesh + 0x528);
...
unsigned __stdcall cache(LPVOID lp)
{
	...
	while (true)
	{
		...
		UWorld = ReadWorld();
		uint64_t persislevel = read<uint64_t>(UWorld + offsets::persistent_level);
		uint64_t gameinstance = read<uint64_t>(UWorld + offsets::game_instance);
		uint64_t localplayer = read<uint64_t>(gameinstance + offsets::local_player_array);
		uint64_t localplayers = read<uint64_t>(localplayer);

		playercontroller = read<uint64_t>(localplayers + offsets::local_player_controller);

		camera_manager = read<uint64_t>(playercontroller + offsets::camera_manager);

		pawn = read<uint64_t>(playercontroller + offsets::local_player_pawn);

		uint64_t actor_array = read<uint64_t>(persislevel + offsets::actor_array);
		int actor_count = read<int>(persislevel + offsets::actor_count);

		uint64_t local_playerstate = read<uint64>(pawn + offsets::player_state);

		uint64_t localroot_component = read<uint64_t>(pawn + offsets::root_component);
		localrelative_component = read<Vector3>(localroot_component + 0x15C);

		uint64_t local_teamcomponent = read<uint64_t>(local_playerstate + offsets::team_component);
		local_teamindex = read<int>(local_teamcomponent + offsets::team_id);
		...
		for (unsigned long i = 0; i < actor_count; ++i)
		{
			...
			uint64_t mesh = read<uint64_t>(actor + offsets::mesh_component);
			uint32_t uniqueid = read<uint32_t>(actor + offsets::unique_id);
			uint64_t damageController = read<uint64_t>(actor + offsets::damage_handler);
			uint64_t playerstate = read<uint64>(actor + offsets::player_state);
...
{
	uint64_t BoneArray = read<uint64_t>(Mesh + offsets::bone_array);
	uint32_t BoneArrayCount = read<uint32_t>(Mesh + offsets::bone_array + 0x8);

	if (!BoneArray)
	{
		BoneArray = read<uint64_t>(Mesh + offsets::bone_array);
		BoneArrayCount = read<uint32_t>(Mesh + offsets::bone_array + 0x8);
	}
...
bool is_visible(uint64_t mesh)
{
	...
	float fLastRenderTimeOnScreen = read<float>(mesh + offsets::last_render_time + 8);
...
inline void RCS(D3DXVECTOR3 Target, D3DXVECTOR3 Camera_rotation, float SmoothFactor) {
	...
	auto ControlRotation = read<D3DXVECTOR3>(playercontroller + 0x3F0);
	...
	*(D3DXVECTOR3*)(playercontroller + offsets::control_rotation) = D3DXVECTOR3(Smoothed.x, Smoothed.y, 0);
}
...
void PredictAim(DWORD_PTR entity, float dist, D3DXVECTOR3 CameraLPos, D3DXVECTOR3 CameraLROT) {
	...
	uint64_t currentactormesh = read<uint64_t>(entity + offsets::mesh_component);

	uint64_t CurrentActorRootComponent = read<uint64_t>(entity + offsets::root_component);
	...
	if (solve(D3DXVECTOR3(CameraLPos.x, CameraLPos.y, CameraLPos.z), weapon, predictor, sol)) {
		...
		*(D3DXVECTOR3*)(playercontroller + offsets::control_rotation) = D3DXVECTOR3(rotpos.x, rotpos.y, 0);
	}
}
void ESP() {
	...
	FCameraCacheEntry camera_cache = read<FCameraCacheEntry>(camera_manager + 0x11E8);

	Vector3 Location = read<Vector3>(camera_manager + offsets::camera_position);
	Vector3 Rotation = read<Vector3>(camera_manager + offsets::camera_rotation);
	float FOV = read<float>(camera_manager + offsets::camera_fov);

	D3DXVECTOR3 LocationX = read<D3DXVECTOR3>(camera_manager + offsets::camera_position);
	D3DXVECTOR3 RotationX = read<D3DXVECTOR3>(camera_manager + offsets::camera_rotation);
	...
	for (unsigned long i = 0; i < entityListCopy.size(); ++i)
	{
		...
		float PlayerHealth = read<float>(entity.DamageController + offsets::health);

		int dormant = read<int>(entity.pObjPointer + offsets::dormant);
		...
		uint64_t rootcomponent = read<uint64_t>(entity.pObjPointer + offsets::root_component);
		Vector3 relativelocation = read<Vector3>(rootcomponent + offsets::root_position);
		...
		uint64_t teamcomponent = read<uint64_t>(entity.PlayerState + offsets::team_component);
		...
		int teamindex = read<int>(teamcomponent + offsets::team_id);

		uint32_t uniqueid = read<uint32_t>(entity.pObjPointer + offsets::unique_id);
...
bool draw_abigsquare()
{
	...
	//this is a paste
	ImGui::Begin("[WehhPaste]", NULL, flags);
	{
